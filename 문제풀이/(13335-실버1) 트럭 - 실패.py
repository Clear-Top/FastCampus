...
trucks = [i for i in map(int, input().rstrip().split(' '))]

Q_truck = deque([trucks[0]])
time = 1
weight = trucks[0]
current_trucks = 1
processed_trucks = 0
...
while True:
    if len(Q_truck) == w:
        ...
        # 다리가 꽉 차지 않은 경우
        if weight + trucks[current_trucks] < L:
            while weight + trucks[current_trucks] <= L and len(Q_truck) + 1 <= w:
                Q_truck.append(trucks[current_trucks])
                weight += trucks[current_trucks]
                current_trucks += 1
                time += 1
            ...
            # 트럭 추가 불가능
            time += w - len(Q_truck) + 1
            weight -= Q_truck.popleft()
            processed_trucks += 1

            # debug
            print(Q_truck)
    if processed_trucks == n:
        break